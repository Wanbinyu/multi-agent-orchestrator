...
- Web 可折叠项目文件树（已在 P2 完成）。
- 跨会话项目索引复用与真实付费模型基准验收。

---

## Phase 6.6 P1 - 项目树与单轮读取缓存 ✅

### 项目结构

- 新增 `project_tree` 注册工具：稳定排序、目录优先、深度/节点上限、隐藏文件开关和重目录忽略规则。
- 新增 CLI `/tree [路径] [深度]`，纯本地执行，不调用 Gateway，不产生 token。
- Windows 带空格路径通过末尾数字解析深度，不使用会破坏反斜杠的 POSIX `shlex`。
- Agent / Worker 项目分析规则改为“先项目树，再抽样入口、依赖、配置、核心模块和测试文件”。
- 项目分析最终答复必须包含“项目结构”章节。
- “只做方案 / 不修改文件”等分析型请求固定走单 Agent，并且原生工具只暴露只读类别。
- `read_file` 设置 12 个不同文件的执行上限；超限请求作为策略跳过，不再继续扩大上下文。
- 超过 6000 字符的项目分析会追加一次无工具精简，保存的最终方案优先保证完整收尾。

### 单轮读取缓存

- 新增 `src/tools/read_cache.py`，为只读工具生成包含执行根目录和规范化参数的稳定缓存键。
- Agent 同步、流式路径和 Worker 多轮工具循环均复用缓存。
- 缓存只保存成功结果；写文件、编辑、命令或未知工具执行后立即失效。
- 流式事件和最终工具记录增加 `cached` 标记，CLI 汇总可显示实际缓存命中数。
- approve 模式下，同一轮已经批准并成功执行的相同读取不会重复询问权限。

### 验证

- 新增 `tests/test_read_cache.py`，扩展项目树、Agent 流式、Worker 和 CLI 命令测试。
- 针对性回归：`56 passed`。
- 真实 `G:\MAO_test` 验收：零 token 项目树 39 个节点；最终分析读取 12 个文件，成本 `$0.017799`，结果 3064 字符且完整结束。
- 全量回归：**395 passed, 1 warning**。

---

## Phase 6.6 P2 - Web 项目文件树 ✅

### 只读浏览 API

- 新增单层目录浏览接口，目录优先稳定排序并复用 `project_tree` 的重目录忽略规则。
- 每个目录最多扫描 5000 项、返回 1000 项，达到上限时返回明确截断标记。
- 新增文本文件预览接口，默认最多返回 20000 字符，只读取所需字节并拒绝二进制文件。

### Web 工作区

- 对话右栏增加“上下文 / 文件”标签页，目录按需展开，不一次性拉取整个项目。
- 支持 Windows 绝对路径、刷新、隐藏文件开关、路径记忆、文件选中和只读预览。
- 修复窄桌面和移动端遮罩覆盖工作区、导致抽屉内容无法点击的问题。

### 验证

- `G:\MAO_test`：10 个根节点，`backend` 展开成功，`README.md` 845 B UTF-8 预览成功。
- 浏览器在 `1280x720`、`390x844` 下无页面横向溢出，控制台无错误。
- JavaScript 语法、Python 编译和 `git diff --check` 通过。
- 全量回归：**399 passed, 1 warning**。
