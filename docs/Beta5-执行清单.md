...
**状态**：B5.1-B5.3 已完成；B5.4 离线基础设施已完成，等待真实评测授权
...
B5.4 已完成离线基础设施、fixed-single/auto-route 小样本和多轮故障收敛；完整回归为 `787 passed, 1 warning`，分发验收通过。hard-attempt guard 与软件 Worker 归一化差异已完成本地审阅：并发预占是原子的，通用角色别名不覆盖合法模型，只有软件任务误用创作角色时才采用软件 Worker 默认模型。只有所有者重新给出新的累计次数边界后，才允许再运行一次 `multi-model` private smoke。详见 [`B5.4-真实能力评测操作手册.md`](B5.4-真实能力评测操作手册.md)。
