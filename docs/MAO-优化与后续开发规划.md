...
**已落地（2026-07-28，模板与渠道）**：
- GitHub Issue 模板：安装反馈、真实任务反馈；强化 Bug / Provider 模板的脱敏确认与错误码字段。
- [`docs/外部用户反馈指南.md`](外部用户反馈指南.md)、[`acceptance/脱敏反馈模板.md`](acceptance/脱敏反馈模板.md)。
- `scripts/sanitize_feedback_text.py` + 单测（Key / Bearer / 用户主目录段脱敏）。

**仍待外部输入**：≥10 安装、≥5 真实任务 Issue；B5.4 真实评测需所有者重新授权。
...
- [x] O4 脱敏反馈模板与渠道（Issue 模板、指南、sanitize 脚本，2026-07-28）。
- [ ] 等待外部用户反馈和新的真实 Provider 授权，不自动恢复付费评测（O4 收集阶段）。
